...
export function ProjectCard({ project, onClick }: ProjectCardProps) {
    ...
    return (
        <div
            onClick={onClick}
            className="group bg-smoke rounded-xl shadow-sm drop-shadow-sm p-6 cursor-pointer hover:shadow-lg hover:-translate-y-1 transition-all duration-200"
        >
            <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2 mb-2">
                        ...
                        {project.isArchived && (
                            <span className="flex items-center gap-1 text-xs text-oyster bg-slate-100 px-2 py-1 rounded-full flex-shrink-0">
                                <Archive size={12} />
                                Archivado
                            </span>
                        )}
            ...
            <div className="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between text-xs text-oyster">
